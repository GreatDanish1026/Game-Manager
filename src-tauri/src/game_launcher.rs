...
fn normalized_store(store: &str) -> String {
    store.trim().to_ascii_lowercase()
}
...
#[cfg(target_os = "windows")]
fn registry_string(root: &RegKey, subkey: &str, value: &str) -> Option<String> {
...
fn steam_candidates() -> Vec<PathBuf> {
    ...
    if let Some(path) = registry_string(&hkcu, r"Software\Valve\Steam", "SteamPath") {
...
fn ea_app_candidates() -> Vec<PathBuf> {
    ...
    if let Ok(program_files) = env::var("ProgramFiles") {
        ...
        candidates.push(root.join("EADesktop.exe"));

        candidates.push(root.join("EALauncher.exe"));
    }
...
fn ubisoft_candidates() -> Vec<PathBuf> {
    ...
    for variable in ["ProgramFiles(x86)", "ProgramFiles"] {
        if let Ok(program_files) = env::var(variable) {
            ...
            candidates.push(root.join("UbisoftConnect.exe"));

            candidates.push(root.join("upc.exe"));
        }
    }
...
fn launcher_status(
    ...
    let path = existing_path(candidates);

    let protocol_ok = protocol.map(protocol_registered).unwrap_or(false);

    let installed = path.is_some() || protocol_ok;
...
fn ea_launcher_status() -> LauncherStatus {
    let path = existing_path(ea_app_candidates());

    let origin2 = protocol_registered("origin2");

    let link2ea = protocol_registered("link2ea");

    let protocol_ok = origin2 || link2ea;

    let installed = path.is_some() || protocol_ok;

    let message = if installed {
        if path.is_some() && protocol_ok {
            "EA app executable and Windows launch protocol detected.".to_string()
        } else if path.is_some() {
            "EA app executable detected.".to_string()
        } else {
            "EA Windows launch protocol detected.".to_string()
        }
    } else {
    ...
    LauncherStatus {
        ...
        launch_method: "origin2:// game launch".to_string(),
...
fn xbox_launcher_status() -> LauncherStatus {
    let script = r#"$gamingApp = Get-AppxPackage -Name Microsoft.GamingApp -ErrorAction SilentlyContinue;
    ...
    LauncherStatus {
        ...
        label: "Xbox / Microsoft Store".to_string(),

        installed: detected,

        path: None,

        protocol_registered: detected,

        launch_method: "Windows package registration / AUMID".to_string(),
...
fn xbox_launcher_status() -> LauncherStatus {
    LauncherStatus {
        ...
        message: "Xbox / Microsoft Store integration is currently available on Windows."
            .to_string(),
    }
}
...
#[tauri::command]
pub fn get_launcher_status() -> Result<Vec<LauncherStatus>, String> {
    Ok(all_launcher_statuses())
}

fn launcher_status_for_store(store: &str) -> Option<LauncherStatus> {
    let normalized = normalized_store(store);

    let id = if normalized == "steam" {
        "steam"
    } else if normalized == "epic" || normalized == "epic games" || normalized == "epic games store"
    {
        "epic"
    } else if normalized == "gog" || normalized == "gog galaxy" {
        "gog"
    ...
    {
        "ea"
    } else if normalized == "ubisoft" || normalized == "ubisoft connect" || normalized == "uplay" {
        "ubisoft"
...
fn steam_id(launcher_id: Option<&str>, game_id: Option<&str>) -> Option<String> {
    let candidate = clean_launcher_id(launcher_id).or_else(|| clean_launcher_id(game_id))?;
...
#[cfg(target_os = "windows")]
fn launch_gog(launcher_id: &str, install_path: &str) -> Result<(), String> {
    let client =
        existing_path(
            gog_galaxy_candidates()
        )
    ...
    Command::new(client)
        .arg("/command=runGame")
        .arg(format!("/gameId={}", launcher_id))
        .arg(format!("/path={}", install_path))
        .spawn()
        .map_err(|error| {
            format!(
...
#[cfg(not(target_os = "windows"))]
fn launch_gog(_launcher_id: &str, _install_path: &str) -> Result<(), String> {
    Err("GOG Galaxy launching is currently implemented for Windows.".to_string())
}
...
pub fn launch_game(
    ...
    let store_normalized = normalized_store(&store);
    ...
    {
        let aumid = clean_launcher_id(launcher_id.as_deref()).ok_or_else(|| {
            "Xbox / Microsoft Store is available, but this game is missing its Windows AUMID."
                .to_string()
        })?;

        crate::xbox_games::launch_xbox_aumid(&aumid)?;

        return Ok(LaunchResult {
            launched: true,

            method: "windows_aumid".to_string(),

            message: "Launch request sent to Windows / Xbox.".to_string(),
        });
    }

    logging::dev_log(&format!("[LAUNCH] Game: {:?}", name));

    logging::dev_log(&format!("[LAUNCH] Store: {:?}", store));

    logging::dev_log(&format!("[LAUNCH] Launcher ID: {:?}", launcher_id));

    if let Some(status) = launcher_status_for_store(&store) {
        if !status.installed {
            return Err(
                format!(
    ...
    if store_normalized == "steam" {
        let app_id = steam_id(launcher_id.as_deref(), game_id.as_deref()).ok_or_else(|| {
            "Steam is available, but this game does not have a valid Steam AppID.".to_string()
        })?;
    ...
    {
        let app_id = clean_launcher_id(launcher_id.as_deref()).ok_or_else(|| {
        ...
        return Ok(LaunchResult {
            ...
            method: "epic_uri".to_string(),

            message: "Launch request sent to Epic Games Launcher.".to_string(),
        });
    }
    ...
    {
        let content_id = clean_launcher_id(launcher_id.as_deref())
            .or_else(|| clean_launcher_id(game_id.as_deref()))
            .ok_or_else(|| {
                "EA app is available, but this game is missing its EA Content ID.".to_string()
            })?;
        ...
        return Ok(LaunchResult {
            ...
            method: "ea_origin2_uri".to_string(),

            message: "Launch request sent to the EA app.".to_string(),
        });
    }
        ...
        || store_normalized == "uplay"
    {
        ...
        let uri = format!("uplay://launch/{}/0", app_id);
        ...
        return Ok(LaunchResult {
            ...
            method: "ubisoft_uri".to_string(),

            message: "Launch request sent to Ubisoft Connect.".to_string(),
        });
    }

    if store_normalized == "gog" || store_normalized == "gog galaxy" {
        let product_id = clean_launcher_id(launcher_id.as_deref()).ok_or_else(|| {
            "GOG Galaxy is available, but this game is missing its GOG product ID.".to_string()
        })?;

        launch_gog(&product_id, &install_path)?;

        return Ok(LaunchResult {
            launched: true,

            method: "gog_galaxy".to_string(),

            message: "Launch request sent to GOG Galaxy.".to_string(),
        });
    }
