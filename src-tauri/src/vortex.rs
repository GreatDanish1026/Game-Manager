...
const VORTEX_REPOSITORY_URL: &str =
...
fn empty_result() -> VortexSupportStatus {
    VortexSupportStatus {
        ...
        page_url: Some(
            VORTEX_REPOSITORY_URL.to_string()
        ),
...
fn remove_common_store_suffixes(
    value: &str,
) -> String {
    let mut value =
        normalize_game_name(
...
fn calculate_match_score(
    ...
    if installed_tokens.is_empty()
        || vortex_tokens.is_empty()
    {
        return 0;
    }
...
fn extension_page_url(
    extension: &VortexExtension,
) -> Option<String> {
    /*
...
#[tauri::command]
pub async fn get_vortex_support(
    name: String,
) -> Result<VortexSupportStatus, String> {
    println!("");
    println!(
        "[VORTEX] ========================================"
    );
    ...
    println!(
        "[VORTEX] Checking game: {:?}",
        name
    );

    println!(
        "[VORTEX] Normalized game: {:?}",
        normalize_game_name(
            &name
        )
    );
    ...
    let client =
        reqwest::Client::builder()
            .user_agent(
                "GameManager/0.3.0 Vortex extension integration"
            )
            .build()
            .map_err(
                |error| {
                    format!(
                        "Failed to create Vortex HTTP client: {}",
                        error
                    )
                }
            )?;
    ...
    println!(
        "[VORTEX] Downloading LIVE Vortex manifest:"
    );
    ...
    let response =
        client
            ...
            .map_err(
                |error| {
                    format!(
                        "Failed to download Vortex extension manifest: {}",
                        error
                    )
                }
            )?;
    ...
    let manifest:
        ...
        response
            ...
            .map_err(
                |error| {
                    format!(
                        "Failed to parse Vortex extension manifest: {}",
                        error
                    )
                }
            )?;
    ...
    let Some(
        ...
        println!(
            "[VORTEX] No matching Vortex game extension found"
        );
