    ...
    let output =
        command
            ...
            .map_err(
                |error| {
                    format!(
                        "Could not start PowerShell for Xbox / Microsoft Store discovery: {error}"
                    )
                }
            )?;
...
#[tauri::command]
pub fn get_xbox_installed_games(
) -> Result<Vec<XboxInstalledGame>, String> {
    /*
...
pub fn launch_xbox_aumid(
    ...
    if value.is_empty() {
        return Err(
            "Xbox / Microsoft Store launch ID is missing."
                .to_string()
        );
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        .map_err(
            |error| {
                format!(
                    "Could not hand Xbox / Microsoft Store launch to Windows: {error}"
                )
            }
        )?;
    ...
    {
        Err(
            "Xbox / Microsoft Store launching is only available on Windows."
                .to_string()
        )
    }
}
