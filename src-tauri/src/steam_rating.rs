...
pub async fn get_steam_community_rating(
    ...
    let summary =
        payload
            .get("query_summary")
            .ok_or_else(
                || "Steam review response did not contain query_summary.".to_string()
            )?;
    ...
    let review_score_percent =
        positive as f64 / total as f64 * 100.0;

    Ok(
        Some(
            SteamCommunityRating {
                positive,
                negative,
                total,
                review_score_percent,
                rating:
                    steamdb_style_rating(
                        positive,
                        negative
                    ),
            }
