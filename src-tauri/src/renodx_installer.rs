...
fn backup_root() -> Result<PathBuf, String> {
    #[cfg(target_os = "windows")]
    {
        let base = env::var_os("LOCALAPPDATA")
            ...
            .map(PathBuf::from)
            .ok_or_else(|| {
                "Could not determine the Windows GameAtlas data directory.".to_string()
            })?;

        return Ok(base.join("GameAtlas").join("renodx-backups"));
    }
...
fn official_package_from_wiki(
    ...
    for line in markdown.lines() {
         ...
         */
        let direct_url = markdown_urls(&parts[2]).into_iter().find(|url| {
        ...
        if asset_name.is_empty() || !asset_name.to_ascii_lowercase().ends_with(suffix) {
            continue;
        }
 ...
 */
fn official_renodx_addon_filename(url: &str, architecture: &str) -> Option<String> {
    ...
    let expected_suffix = if architecture == "32-bit" {
        ".addon32"
    } else {
        ".addon64"
    };
...
fn official_renodx_manual_message(matched_name: Option<&str>, source_url: Option<&str>) -> String {
    let title = matched_name
...
pub async fn get_renodx_package_info(
     ...
     */
    let official_lookup_name = renodx_match_name
        .clone()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| game_name.clone());

    let official_status =
        crate::renodx::get_renodx_mod_status(official_lookup_name.clone()).await?;

    if official_status.renodx.found {
        let official_matched_name = official_status
            .renodx
            .matched_name
            .clone()
            .or_else(|| Some(official_lookup_name.clone()));

        if let Some(official_url) = official_status.renodx.download_url.clone() {
            if let Some(official_asset_name) =
                official_renodx_addon_filename(&official_url, &architecture)
            {
                return Ok(PackageInfo {
                    found: true,
                    release_tag: Some("official-mods-list".to_string()),
                    release_name: official_matched_name,
                    asset_name: Some(official_asset_name.clone()),
                    asset_url: Some(official_url),
                    asset_size_bytes: None,
                    match_type: "official-direct".to_string(),
                    message: format!(
            ...
            return Ok(PackageInfo {
                found: false,
                release_tag: Some("official-mods-list".to_string()),
                release_name: official_matched_name.clone(),
                asset_name: None,
                asset_url: None,
                asset_size_bytes: None,
                match_type: "official-manual".to_string(),
                message: official_renodx_manual_message(
                    official_matched_name.as_deref(),
                    Some(official_url.as_str()),
                ),
            });
        }
        ...
        return Ok(PackageInfo {
            ...
            asset_size_bytes: None,
            match_type: "official-manual".to_string(),
            message: official_renodx_manual_message(official_matched_name.as_deref(), None),
        });
    }
     ...
     */
    if let Some(package) =
        official_package(&game_name, renodx_match_name.as_deref(), &architecture).await?
    {
        return Ok(package);
    }
...
pub async fn uninstall_renodx_package_linux(
    ...
    let backup = backup_existing(&destination, &game_key)?;
    fs::remove_file(&destination).map_err(|e| format!("Could not remove RenoDX file: {}", e))?;
...
fn latest_renodx_backup(game_key: &str) -> Result<Option<(u64, PathBuf)>, String> {
...
fn validate_renodx_filename(asset_name: &str) -> Result<&str, String> {
    if asset_name.trim().is_empty() {
    ...
    if file_name != asset_name {
        return Err("RenoDX restore refused a filename containing a path.".to_string());
    }
...
#[tauri::command]
pub fn get_latest_renodx_backup(game_key: String) -> Result<RenoDxBackupInfo, String> {
    let Some((created_unix, path)) = latest_renodx_backup(&game_key)? else {
    ...
    Ok(RenoDxBackupInfo {
        ...
        message: format!("Previous RenoDX backup is available: {}.", asset_name),
    })
}
...
pub fn restore_latest_renodx_backup_linux(
    ...
    let Some((_created_unix, backup_path)) = latest_renodx_backup(&game_key)? else {
        return Err("No RenoDX backup is available for this game.".to_string());
    };
    ...
    if let Some(current_name) = current_asset_name.as_deref() {
        ...
        if current_path.is_file() && current_path != destination {
            safety_backup = backup_existing(&current_path, &game_key)?;

            fs::remove_file(&current_path).map_err(|e| {
                format!("Could not remove current RenoDX file before restore: {}", e)
            })?;
        }
    }

    if destination.is_file() {
        let same_name_backup = backup_existing(&destination, &game_key)?;
    ...
    let temporary = target.join(format!("{}.gameatlas-restore-tmp", backup_name));
    ...
    if temporary_size != backup_size {
        ...
        return Err("RenoDX restore verification failed before commit.".to_string());
    }
    ...
    if final_size != backup_size {
        return Err("RenoDX restore verification failed after commit.".to_string());
    }
    ...
    Ok(RestoreResult {
        ...
        safety_backup_path: safety_backup.map(|path| path.to_string_lossy().to_string()),
        message: format!(
...
pub async fn get_renodx_update_status_linux(
    ...
    let available_name = package
        .asset_name
        .clone()
        .ok_or_else(|| "RenoDX package filename is missing.".to_string())?;

    validate_renodx_filename(&available_name)?;

    let official_local_path = target.join(&available_name);

    let comparison_name = if official_local_path.is_file() {
        available_name.clone()
    } else if let Some(detected_name) = installed_asset_name {
        validate_renodx_filename(&detected_name)?.to_string()
    } else {
        return Ok(RenoDxUpdateStatus {
            checked: true,
            state: "not-installed".to_string(),
            installed_asset_name: None,
            available_asset_name: Some(available_name),
            installed_size_bytes: None,
            available_size_bytes: package.asset_size_bytes,
            message: "RenoDX is not currently installed.".to_string(),
        });
    };

    let installed_path = target.join(&comparison_name);

    if !installed_path.is_file() {
        return Ok(RenoDxUpdateStatus {
            checked: true,
            state: "not-installed".to_string(),
            installed_asset_name: Some(comparison_name),
            available_asset_name: Some(available_name),
            installed_size_bytes: None,
            available_size_bytes: package.asset_size_bytes,
            message: "GameAtlas no longer finds the detected RenoDX addon.".to_string(),
        });
    }

    let installed_size = fs::metadata(&installed_path)
        .map_err(|e| format!("Could not inspect installed RenoDX addon: {}", e))?
        .len();

    let asset_url = package
        .asset_url
        .clone()
        .ok_or_else(|| "RenoDX package URL is missing.".to_string())?;

    let expected_size = package.asset_size_bytes.unwrap_or(0);

    let available_bytes = reqwest::Client::builder()
        .user_agent("GameAtlas/2.1 RenoDX Manager")
        .build()
        .map_err(|e| e.to_string())?
        .get(&asset_url)
        ...
        .error_for_status()
        .map_err(|e| format!("Current RenoDX build returned an error: {}", e))?
        .bytes()
        .await
        .map_err(|e| format!("Could not read current RenoDX build: {}", e))?;

    if available_bytes.len() < 100_000 {
        return Err(format!(
    ...
    if expected_size > 0 && available_bytes.len() as u64 != expected_size {
        return Err(format!(
    ...
    let available_size = available_bytes.len() as u64;

    let is_current = if installed_size != available_size {
        false
    } else {
        let installed_bytes = fs::read(&installed_path).map_err(|e| {
            format!(
        ...
        installed_bytes.as_slice() == available_bytes.as_ref()
    };

    if is_current {
        Ok(RenoDxUpdateStatus {
            checked: true,
            state: "up-to-date".to_string(),
            installed_asset_name: Some(comparison_name),
            available_asset_name: Some(available_name),
            installed_size_bytes: Some(installed_size),
            available_size_bytes: Some(available_size),
            message: "Installed RenoDX exactly matches the current official build.".to_string(),
        })
    } else {
        Ok(RenoDxUpdateStatus {
            checked: true,
            ...
            installed_size_bytes: Some(installed_size),
            available_size_bytes: Some(available_size),
            message: "A newer or different official RenoDX build is available.".to_string(),
        })
    }
}
