...
fn parse_renodx_status(
        ...
        markdown.lines()
    {
        ...
        let status =
            if full_row.contains(
                ":white_check_mark:"
            ) {
                "working"
            } else if full_row.contains(
                ":construction:"
            ) {
                "in_progress"
            } else {
...
#[tauri::command]
pub async fn get_renodx_mod_status(
    name: String,
) -> Result<HdrModStatus, String> {
    println!("");
    println!(
        "[HDR MODS] ====================================="
    );
    ...
    let client =
        reqwest::Client::builder()
            .user_agent(
                "GameManager/0.3.0 HDR mod integration"
            )
            .build()
            .map_err(|error| {
                format!(
