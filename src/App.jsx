...
async function getLinuxIntegratedGames(installedGames) {
  ...
  if (
    String(platform?.hostPlatform ?? "").toLowerCase() !== "linux"
  ) {
    return installedGames;
  }
  ...
  try {
    const [
      ...
      invoke("get_linux_steam_games"),
                                 invoke("get_heroic_installed_games"),
                                 invoke("get_lutris_installed_games"),
    ]);
    ...
    const heroicGames =
    heroicResult.status === "fulfilled"
    ? heroicResult.value
    : [];

    const lutrisGames =
    lutrisResult.status === "fulfilled"
    ? lutrisResult.value
    : [];

    if (heroicResult.status === "rejected") {
      console.warn(
    ...
    if (lutrisResult.status === "rejected") {
      console.warn(
    ...
    console.warn(
      "[GameAtlas] Linux Steam discovery failed:",
      error
    );
